...
**Optional — enable field reports (`report_issue`).** No code change required. To let proven
patrons file field reports about your tools as GitHub issues on your repo, just deliver two
secrets on your operator credential service via Secure Courier:
...
The runtime auto-includes these optional fields in your courier template, so no template
edit is needed. The `<slug>_report_issue` tool is always registered; without the two secrets
it simply returns an "issue reporting not configured" situation. It is proof-gated and priced
(seeds at 1 sat) so it cannot be spammed, and refunds automatically when nothing is filed. The
issue's author of record is the caller's npub, stamped into the body.
